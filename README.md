...
ComfyUI のワークフローを補完・拡張する統合ブリッジモジュールです。Pipe 形式によるコンテキスト管理、Wildcard/LoRA 対応プロンプト処理、高精度な Detailer、画像アップスケール、推論高速化キャッシュ、最終出力処理、マスク付きノイズ注入、シーンベースのトグル管理の 8 機能を提供します。

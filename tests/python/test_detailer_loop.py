...
class TestLatentPersistent:
    """cycle 間で latent を保持し VAE 往復を 1 回に抑える挙動の検証。"""

    def test_latent_chains_across_cycles(self):
    ...
    def test_latent_noise_decays_with_denoise_decay(self):
        # latent_noise_intensity が denoise_decay と連動して減衰する
        images = torch.rand(1, 64, 64, 3)
        mask = _make_mask(1, 64, 64)
        vae = _make_vae()
        captured_inputs = []

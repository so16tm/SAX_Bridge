...
### マスクを用途別に調整 — Mask Adjust

**SAX Mask Adjust** は入力 MASK に対して **拡張・収縮 → ぼかし → 二値化** を一括で適用する単機能ノードです。同じ SAM3 出力を「Detailer には大きめ / NoiseInjector には小さめ」と用途ごとに分岐したいときに、SAM3 を複数本走らせる必要がなくなります。

- `invert`：「白く塗った領域 = 加工対象」を「保護対象」に切り替え（SAM3 で検出した顔を保護して背景を Detailer 等）
- `grow`：エッジが角張った高速な拡張・収縮（SAM3 と同じセパラブル MaxPool 実装）
- `blur` + `threshold`：ジャギのない滑らかな膨張/収縮（`blur=3.0, threshold=0.1` で滑らか拡張、`0.9` で滑らか収縮）
- `blur` のみ：境界羽化のソフトマスク化（KSampler の noise_mask 用）

SAM3 側の `mask_grow` と本ノードの `grow` は加算的に効くため、後段で分岐する場合は SAM3 側 `mask_grow=0` にして本ノードで調整するのが分かりやすい設計です。
